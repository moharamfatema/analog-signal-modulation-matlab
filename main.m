...
plotTimeFrequency(m, " Message Signal", 2, 1, 1)
sound(m, fs);
...
plotTimeFrequency(dsbsc, " DSB-SC", 2, 1, 1)
plotTimeFrequency(dsbtc, " DSB-TC", 2, 1, 1)

[msc, mtc] = AMreceiver(dsbsc,dsbtc,info,fc);
plotTimeFrequency(msc, " Received signal from DSB-SC", 2, 1, 1)
sound(msc,fs);

plotTimeFrequency(mtc, " Received signal from DSB-TC", 2, 1, 1)
sound(2.*mtc,fs);
...
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with no noise", 2, 1, 1)
sound(demodSig, fs);

%Coherent detection of DSB-SC with SNR = 0dB
noisySig1 = awgn(dsbsc, 0, 'measured', 'dB');
demodSig = coherentDetector(noisySig1, info, fc, 0);
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with SNR = 0dB", 2, 1, 1)
sound(demodSig, fs);
...
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with SNR = 10dB", 2, 1, 1)
sound(demodSig, fs);
...
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with SNR = 30dB", 2, 1, 1)
sound(demodSig, fs);
...
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with frequency error, fc = 100.1KHz", 2, 1, 1)
sound(demodSig, fs);

%Coherent detection of DSB-SC with phase error = 20 degrees
demodSig = coherentDetector(dsbsc, info, fc, 20 * pi / 180);
plotTimeFrequency(demodSig, " Coherent detection of DSB-SC with phase error = 20 degrees", 2, 1, 1)
sound(demodSig, fs);
